...
public class CTRemoveQMDRecipe implements IAction
{
	...
	public CTRemoveQMDRecipe(QMDRecipeHandler recipeHandler, IngredientSorption type, List<IIngredient> ctIngredients)
	{
		...
		{
			CraftTweakerAPI.logError("A " + recipeHandler.getName() + " recipe was the wrong size");
			wrongSize = true;
			return;
		}
	...
	public String describe()
	{
		if (ingredientError || wasNull || wrongSize)
		{
			...
			return String.format("Error: Failed to remove %s recipe with %s as the " + (type == IngredientSorption.INPUT ? "input" : "output"), recipeHandler.getName(), QMDRecipeHelper.getAllIngredientNamesConcat(itemIngredients, fluidIngredients, particleIngredients));
		}
		if (type == IngredientSorption.INPUT)
		{
			return String.format("Removing %s recipe: %s", recipeHandler.getName(), QMDRecipeHelper.getRecipeString(recipe));
		}
		else return String.format("Removing %s recipes for: %s", recipeHandler.getName(), QMDRecipeHelper.getAllIngredientNamesConcat(itemIngredients, fluidIngredients, particleIngredients));
	}
