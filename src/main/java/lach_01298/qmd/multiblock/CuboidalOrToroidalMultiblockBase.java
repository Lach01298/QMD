...
public abstract class CuboidalOrToroidalMultiblockBase<PACKET extends MultiblockUpdatePacket> extends MultiblockBase<PACKET>
{

	private int thickness;
	protected CuboidalOrToroidalMultiblockBase(World world, int thickness)
	{
		super(world);
	...
	protected boolean isMachineWhole(IMultiblockValidator validatorCallback)
	{
		...
		{
			validatorCallback.setLastError("zerocore.api.nc.multiblock.validation.machine_too_large", null, maxXSize,
					"X");
			return false;
		}
		if (maxYSize > 0 && deltaY > maxYSize)
		{
			validatorCallback.setLastError("zerocore.api.nc.multiblock.validation.machine_too_large", null, maxYSize,
					"Y");
			return false;
		}
		if (maxZSize > 0 && deltaZ > maxZSize)
		{
			validatorCallback.setLastError("zerocore.api.nc.multiblock.validation.machine_too_large", null, maxZSize,
					"Z");
			return false;
		}
		if (deltaX < minXSize)
		{
			validatorCallback.setLastError("zerocore.api.nc.multiblock.validation.machine_too_small", null, minXSize,
					"X");
			return false;
		}
		if (deltaY < minYSize)
		{
			validatorCallback.setLastError("zerocore.api.nc.multiblock.validation.machine_too_small", null, minYSize,
					"Y");
			return false;
		}
		if (deltaZ < minZSize)
		{
			validatorCallback.setLastError("zerocore.api.nc.multiblock.validation.machine_too_small", null, minZSize,
					"Z");
			return false;
		}
		...
		TileEntity te;
		CuboidalOrToroidalMultiblockTileBase part;
		Class<? extends CuboidalOrToroidalMultiblockBase> myClass = getClass();
		int extremes;
		boolean isPartValid;
		...
		for (int x = minX; x <= maxX; x++)
		{
			for (int y = minY; y <= maxY; y++)
			{
				for (int z = minZ; z <= maxZ; z++)
				{
					...
					if (te instanceof CuboidalOrToroidalMultiblockTileBase)
					{
						part = (CuboidalOrToroidalMultiblockTileBase) te;
