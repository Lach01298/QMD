		...
		if (player != null && player.getHeldItem(hand).isEmpty()) 
		{
			...
			if (!world.isRemote && tile instanceof TileAtmosphereCollector) 
			{
				...
				if(fluid != null)
				{
					...
					player.sendMessage(new TextComponentString(Lang.localise("message.qmd.atmosphere_collector",Lang.localise(name),Units.getSIFormat(collector.getCollectionRate(), -3, "B/t"))));
				}
				else
				{
					player.sendMessage(new TextComponentString(Lang.localise("message.qmd.atmosphere_collector_no_fluid")));
				}
